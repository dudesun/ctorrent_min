...
int Ctcs::Send_Config()
{
  ...
  if( m_protocol >= 3 ){
    ...
    double num = BTCONTENT.GetSeedTime() ?
        (cfg_seed_mins - (now - BTCONTENT.GetSeedTime())/60.0) :
        cfg_seed_mins;
    unsigned long tmp = (unsigned long)(num * 100);
    int pre = (tmp % 10) ? 2 : (tmp % 100) ? 1 : 0;
    snprintf(value, MAXPATHLEN, "%.*f", pre, num);
    snprintf(desc, MAXPATHLEN, "~mins remaining (-e %lu)", 
      (unsigned long)cfg_seed_mins);
    ...
    snprintf(message, CTCS_BUFSIZE, "CTCONFIG %d %d %f %d %d %d %d %d",
      (int)arg_verbose, (int)cfg_seed_mins, cfg_seed_ratio,
  ...
  if( m_protocol >= 3 ){
    ...
    if( 0==strcmp(name, "verbose") ){
      ...
      if( 0==BTCONTENT.GetSeedTime() ||
          cfg_seed_mins > (now - BTCONTENT.GetSeedTime()) / 60 ||
    ...
    if(msgbuf[11] != '.') cfg_seed_mins = atoi(msgbuf+11);
