    ...
    if( K_LINES==m_streams[O_INPUT]->GetInputMode() ){  // command parameter
      ...
      case '-':				// decrease value
        ...
        case 'u': cfg_max_bandwidth_up +=
          ...
          break;
        case 'e': cfg_seed_mins += inc;
          if( cfg_seed_mins < 0 ) cfg_seed_mins = 0;
          break;
      ...
      case 'u': InteractU("UL Limit: %d B/s ", (int)cfg_max_bandwidth_up);
        break;
      case 'e': InteractU("Seed time: %.1f mins ", BTCONTENT.GetSeedTime() ?
          (cfg_seed_mins - (now - BTCONTENT.GetSeedTime())/(double)60) :
          (double)cfg_seed_mins);
        break;
  ...
  if( 0==oper_mode ){
    ...
    if( sel <= O_NCHANNELS+1 ){  // change i/o channel
      ...
      Interact("Ratio: %.2f   Seed time: %luh   Seed ratio: %.2f",
        ...
        (unsigned long)cfg_seed_mins, cfg_seed_ratio);
...
void Console::StatusLine1(char buffer[], size_t length)
{
  ...
  if( !BTCONTENT.Seeding() || BTCONTENT.FlushFailed() ){  // downloading
    ...
    if( cfg_seed_mins )
      remain = cfg_seed_mins - (now - BTCONTENT.GetSeedTime()) / 60;
